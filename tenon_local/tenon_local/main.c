...
static jconf_t *global_conf = NULL;
static uint64_t today_used_bandwidth = 0;
static uint64_t today_bandwidth_tm = 0;
static uint64_t prev_write_bandwidth_tm = 0;
...
static unsigned int ex_route_ip __attribute__((unused)) = 0;
static unsigned short ex_route_port __attribute__((unused)) = 0;
...
void my_split( char **arr, char *str, const char *del) {
    char *s =NULL;
    s=strtok(str,del);
    while(s != NULL)
    {
...
static uint64_t get_now_timestamp_milli() {
    struct timeval tv;
...
static void read_use_bandwidth() {
    if (global_conf->bandwidth_file == NULL) {
        return;
    }
    
    FILE* fd = fopen(global_conf->bandwidth_file, "r");
    if (fd == NULL) {
        return;
    }
    
    char str[1024];
    size_t read_len = fread(str, 1, 1024, fd);
    
    const char *del = ",";
    char *arr[2];
    int num_count = strsplinum(str,del);
    my_split(arr, str, del);
    
    if (num_count == 2) {
        today_bandwidth_tm = atoll(arr[0]);
        today_used_bandwidth = atoll(arr[1]);
        uint64_t now_day_tm = get_now_timestamp_milli() / (24llu * 3600llu * 1000llu);
        if (today_bandwidth_tm != now_day_tm) {
            today_bandwidth_tm = now_day_tm;
...
static void write_use_bandwidth(int64_t bandwidth) {
    today_used_bandwidth += bandwidth;
    uint64_t now_tm = get_now_timestamp_milli();
    uint64_t now_day_tm = now_tm / (24llu * 3600llu * 1000llu);
    if (today_bandwidth_tm != now_day_tm) {
        today_bandwidth_tm = now_day_tm;
        today_used_bandwidth = 0;
    }
    
    if (now_tm - prev_write_bandwidth_tm < 3000) {
        return;
    }
    
    prev_write_bandwidth_tm = now_tm;
    if (global_conf->bandwidth_file == NULL) {
        return;
    }
    
    FILE* fd = fopen(global_conf->bandwidth_file, "w");
    if (fd == NULL) {
        return;
    }
    ...
    snprintf(str, 1024, "%llu,%llu", today_bandwidth_tm, today_used_bandwidth);
        ...
        if (!fast_open || remote->direct) {
            ...
            if (s == 0) {
                ...
                if (use_smart_route != 0) {
                    // send vpn server ip
                    if (ex_route_ip > 0 && ex_route_port > 0) {
                        ip_port_t vpn_info;
                        vpn_info.ip = ex_route_ip;
                        vpn_info.port = htons(ex_route_port);
                ...
                char* plat_and_ver = "mac_5.0.1";
                uint8_t m_len = (uint8_t)(strlen(plat_and_ver));
                server_head[start_pos + 66] = m_len;
                memcpy(server_head + start_pos + 67, plat_and_ver, m_len);
...
{
    if (connect_times > 5) {
        printf("connect times too mutch and exit.\n");
        connect_times = 0;
        //exit(0);
    }
...
static void* update_config_thread(void* test) {
    if (svr_conf_path != NULL) {
        while (!stoped) {
            ...
            route_ip_arr[invalid_idx]->ex_route_ip = conf.ex_route_ip;
            route_ip_arr[invalid_idx]->ex_route_port = conf.ex_route_port;
...
main(int argc, char **argv)
{
    ...
    if (conf_path != NULL) {
        ...
        ex_route_ip = conf->ex_route_ip;
        ex_route_port = conf->ex_route_port;
        vpn_ip = conf->vpn_ip;
        ...
        route_ip_arr[0]->ex_route_ip = ex_route_ip;
        route_ip_arr[0]->ex_route_port = ex_route_port;
        route_ip_arr[1]->ip = route_ip;
        route_ip_arr[1]->port = route_port;
        route_ip_arr[1]->ex_route_ip = ex_route_ip;
        route_ip_arr[1]->ex_route_port = ex_route_port;
              
        LOGI("use smart route[%d], route_ip[%u], route port[%d], ex_route_ip[%u], ex_route port[%d]"
            "vpn_ip[%u], vpn_port[%d], seckey[%s], pubkey[%s], method[%s]",
            use_smart_route, route_ip, route_port, ex_route_ip, ex_route_port, vpn_ip, vpn_port, seckey, pubkey, enc_method);
    }
