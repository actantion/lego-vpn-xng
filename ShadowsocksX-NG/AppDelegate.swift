...
class AppDelegate: NSObject, NSApplicationDelegate, NSUserNotificationCenterDelegate {
    ...
    func applicationDidFinishLaunching(_ aNotification: Notification) {
        ...
        let res = TenonP2pLib.sharedInstance.InitP2pNetwork(local_ip, 7982)
...
//        ShortcutsController.bindShortcuts()
    ...
    func doToggleRunning(showToast: Bool) {
        ...
        if showToast {
            if isOn {
                self.makeToast("TenonVPN: On".localized)
            }
            else {
                self.makeToast("TenonVPN: Off".localized)
            }
        }
    }
...
//
    ...
    func updateMainMenu() {
        ...
        let isOn = defaults.bool(forKey: "ShadowsocksOn")
        if isOn {
            runningStatusMenuItem.title = "TenonVPN: On".localized
            runningStatusMenuItem.image = NSImage(named: NSImage.Name(rawValue: "NSStatusAvailable"))
            toggleRunningMenuItem.title = "Turn TenonVPN Off".localized
            let image = NSImage(named: NSImage.Name(rawValue: "menu_icon"))
            statusItem.image = image
        } else {
            runningStatusMenuItem.title = "TenonVPN: Off".localized
            toggleRunningMenuItem.title = "Turn TenonVPN On".localized
